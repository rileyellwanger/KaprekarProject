...
int isKaprekar(int n) {
  ...
  if(n < 1) {
    return 0; //computer recognizes 0 as false
  }
  ...
  int numberOfDigits = (int) log10(n) + 1; //matched variable with variable on line 40
  long modulus = 0;
  long first, second;
  ...
  for(i=1; i<=numberOfDigits; i++) {
    ...
    if(second > 0 && n == first + second) { //added second equal sign
      return 1;
    }
  }
  return 0;
